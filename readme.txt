...
* Change post type — switch a post to a page, or a page to a post, straight from the Posts and Pages lists (a bulk action and Quick Edit) and from the block or classic editor. Converting a post to a page can assign a parent page; converting a page to a post can assign a category. When the URL changes a 301 redirect is created automatically, a converted page's child pages are re-parented so nothing is orphaned, and the whole change can be undone from a notice. The front page and posts page are protected, and the feature can be turned off.
...
* Heading anchors — every content heading gets a stable "jump-" prefixed id, set on the heading block as you write so you can link to it from elsewhere in the post straight away with the normal link tool. Anything published without one still gets its anchor when the page is served, and your own HTML anchors are always kept. One checkbox, on by default.
* Breadcrumbs — an accessible breadcrumb trail following the WAI-ARIA pattern (a labeled nav, an ordered list, and a plain non-linked current crumb marked with aria-current), with matching BreadcrumbList schema. Posts follow their category structure, pages their parent/child structure. Turn it on under Site Details, then place it with the Breadcrumbs block, the [coywolf_seo_breadcrumbs] shortcode, or the coywolf_seo_breadcrumbs() template tag. Pick the separator, name the home crumb, show or hide the home and current crumbs, and style everything through the coywolf-seo-breadcrumbs classes.
...
* AI enrichment — bring your own API key for Claude (Anthropic), OpenAI, or Google Gemini and it grounds each post's main subjects to real Wikidata entities in the Article schema's about, mentions, and sameAs (identifiers are looked up, never invented). Enrich-all runs use the Batch API at roughly half price with live progress and a cost readout, and optional AI meta descriptions write a short summary per post, and fill an empty excerpt with it too. Uses WordPress 7.0's bundled AI client and requires WordPress 7.0 or higher.
...
* Redirects — a full redirect manager on one screen: quick-add exact or regex rules (301/302/307/308/410), test any URL before trusting the live site, and watch hit counts. Change a published post's slug and the editor offers the 301 old-to-new the moment you save; trash a post and the Posts list asks whether to redirect or 410 its URL. It refuses to save a redirect that would loop, and breaks any redirect loop at serve time instead of trapping the visitor. When the Redirection plugin is active, Coywolf SEO takes over URL redirects and points you to import its rules.
...
The SEO Description is used as the excerpt too. A manual excerpt always wins; if you set an SEO Description but leave the excerpt blank, that description becomes the excerpt as well as the meta and Open Graph description, so all three stay consistent. Neither ever overwrites an excerpt you wrote yourself — they only fill a blank one. If you have AI meta descriptions turned on, the AI-written summary fills a blank excerpt in the same way when the post is published.
...
= 1.1.12 =
* Confirm WordPress 7.1 compatibility (Tested up to: 7.1) (#143).

= 1.1.11 =
* Add Post ↔ Page type switcher (#142).

= 1.1.10 =
* Add accessible breadcrumb navigation (#141).
...
= 1.1.6 =
* Editor-side heading anchors, Excerpt from the Description, and slug-change redirects on the post (#137).
