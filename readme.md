...
- **Meta description** — by default the homepage uses your description (default: the Tagline), posts and pages use the per-post SEO Description when you set one and otherwise their manual excerpt, and terms use their description; nothing is auto-generated unless you turn on AI meta descriptions (see *AI Schema enrichment*). A per-post SEO Description also becomes the post's excerpt when it has none of its own, so the excerpt, meta description, and Open Graph description stay in step. One checkbox excludes meta descriptions entirely — search engines generate snippets from content anyway.
...
<!-- labs-strip:start -->
- **Labs → AI Discovery** *(experimental, off by default)* — turn AI Discovery on from the **Labs** page, then choose which of three "make this site discoverable to AI" outputs to publish. Each output is off until you enable it, all three are built entirely from data already on your site (no new external calls), and turning AI Discovery off withdraws every output at once. Because the OKF and EntityMap outputs are built from the AI entity enrichment, the panel shows whether enrichment still needs running for your content and roughly what it costs — it only needs to be run once; every new publish and update to a post or page is enriched automatically and keeps the outputs current. If the official [WordPress MCP Adapter](https://github.com/WordPress/mcp-adapter) plugin is installed, the published outputs are additionally exposed as read-only **MCP tools** at `/wp-json/coywolf-seo/v1/mcp` (for clients like Claude Desktop, Claude Code, and Cursor, scoped to the connecting user), with a per-tool allow-list on the Labs page; the adapter is detected at runtime and never bundled. The whole feature ships in the GitHub distribution only. The three outputs:
  - **Open Knowledge Format (OKF) export** — generates an [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) v0.1 bundle of your public content: a navigable graph of Markdown concepts for every published article, topic, and author, cross-linked to the AI-enriched entities each page is *about* or *mentions*, each grounded to its Wikidata QID and Wikipedia page. Download the bundle as a `.zip` or a gzipped `.tar.gz` tarball, or let an agent traverse it live at a `/okf/` read endpoint; it rebuilds in the background as content changes. An optional **Advertise** sub-setting points agents at the `/okf/` root from a referenced `llms.txt`, a `<link rel="alternate">` in the page `<head>`, and a `robots.txt` allowance — never overwriting an `llms.txt` or `robots.txt` owned elsewhere.
  - **EntityMap** — publishes a spec-conformant [EntityMap v1.0](https://entitymap.org/spec/v1.0) file set (`entitymap.json` plus a human/crawler-readable `entitymap.html`) from your domain root, indexing the Wikidata-grounded entities your pages are *about* or *mention* with extractive, publisher-attributed evidence passages so the attribution survives plain-text ingestion. The automated output is marked `verificationStatus: "generator-draft"`. Download as a `.zip` or read it live; an optional **Advertise** sub-setting points agents at it from the channels the spec lists (`llms.txt`, a `<link rel="entitymap">`, a `robots.txt` allowance).
  - **Agentic Resource Discovery (ARD)** — publishes an [Agentic Resource Discovery](https://agenticresourcediscovery.org/) `ai-catalog.json` manifest at `/.well-known/ai-catalog.json` so AI clients and registries can discover the machine-usable resources your site offers. Unlike OKF and EntityMap (which describe your *content* and *entities*), ARD catalogs *invocable / connectable resources*: by default your WordPress REST API plus whichever of the OKF, EntityMap, and `llms.txt` outputs are turned on, and optionally an MCP server card and any custom entries you add (a JSON list of `{displayName, type, url}` objects, e.g. an A2A agent card or an API). Each entry carries a domain-anchored `urn:air:` identifier and natural-language *representative queries*. The manifest is served dynamically with the spec's CORS header — **no file is written to your site root** — and an optional **Advertise** sub-setting adds a `<link rel="ai-catalog">` and a `robots.txt` allowance, and shows the exact `Agentmap:` line and DNS record to add by hand. ARD describes resources an agent can *use*, not page content, and registry adoption is still early — so it is honest, forward-looking advertising.
<!-- labs-strip:end -->
