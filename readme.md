...
- **Mobile alternative image** — the core Image block gains an *Add mobile alternative* control: pick a phone-specific image (a different crop, dimensions, or content) and the plugin swaps it into the small `300w` and `768w` `srcset` candidates on the front end, so phones download the alternative while larger screens keep the original. The desktop image, its lightbox, and every other breakpoint are left untouched.
